...
export default {
  ...
  props: {
    ...
    // scrollThreshold 可以设置到第几个标签的时候开始滚动
    scrollThreshold: {
      type: Number,
      default: 4
    },
  ...
  computed: {
    barLeft () {
      if (this.hasReady) {
        ...
        // getBoundingClientRect() 可以返回元素的宽高偏移等css
        ...
        const count = this.scrollable ? (this.boxWidth / this.$children[this.currentIndex || 0].$el.getBoundingClientRect().width) : this.number
