...
<script>
import { parentMixin } from '../../../node_modules/vux/src/mixins/multi-items'
...
export default {
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      ...
      this.innerStyle = 'width:' + this.innerStyleWidth + 'px'
    })
  },
  props: {
    lineWidth: {
    ...
    // scrollThreshold 可以设置几个标签的时候开始滚动
    scrollThreshold: {
      type: Number,
      default: 4
    },
  ...
  computed: {
    ...
    scrollable () {
      // this.number 标签的总个数
      // this.scrollThreshold 一屏可以放几个标签
    ...
    innerStyleWidth () {
      ...
      for (let i = 0; i < this.number; i++) {
        styleWidth = styleWidth + document.getElementsByClassName('vux-tab-item')[i].getBoundingClientRect().width
      }
      return styleWidth
    }
  },
  ...
  methods: {
    // 控制超出屏幕后导航向做滚动，以便显示出后面的tab-item
    scrollToActiveTab () {
      ...
      const step = () => {
        ...
        if (++count < scrollDuration) {
          window.requestAnimationFrame(step) // 告诉浏览器您希望执行动画并请求浏览器在下一次重绘之前调用指定的函数来更新动画
        }
      }
